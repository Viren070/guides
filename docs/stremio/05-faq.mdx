...
**However, you cannot use the service from more than one public IP address at a time.**
...
These are available [here](guide#which-debrid-service-do-i-use).
:::
...
<Tabs groupId="platform" className="custom-tabs">
  <TabItem value="web" label="Stremio Web / iOS">
    ...
    2. Click the ` + Add Addon` button 3. Paste the URL into the box and click
    `Add` 4. A prompt will appear asking if you want to install the addon. Click
    `Install`
  </TabItem>
  <TabItem value="app" label="Stremio Apps">
    This is for if you are using the Stremio app on a device such as Android,
    Windows, macOS etc. 1. Open the Stremio app and go to the Addons menu. 2.
    Click the search bar and paste the URL into the search bar. 3. A prompt will
    appear asking if you want to install the addon. Click `Install`
  </TabItem>
</Tabs>
...
You will need to set an external player such as Outplayer or VLC. Follow this [blog post from Stremio](https://blog.stremio.com/using-stremio-web-on-iphone-ipad/) for more information.
