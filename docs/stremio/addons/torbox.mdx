...
4. Scroll down to the `integrations`. 
...
6. Now that the addon is installed, we can configure it. This addon is different to the rest, as the configuration can be done from the Torbox website. There is no need to reinstall the addon after configuring. 
