...
<TabItem value="ios" label="iOS">
    ...
    The stremio server by default, can only be accessed locally. To access it from another device (in the same network) follow these steps:

    1. Make sure the Stremio service is running, then open Stremio Web on the same device 
    2. Go to the settings page,  then click the `Streaming` section. 
    3. Set the Streaming HTTPS endpoint to your local IP address (this option should appear as a dropdown).
    4. Then copy the Remote URL and paste it into your other iOS device's Stremio settings for the streaming server URL.
...
<TabItem value="rasberry" label="Rasberry Pi 4 and 5">
...
- [Real Debrid](https://real-debrid.com/) (Consider using [this affiliate link](http://real-debrid.com/?id=9483829) to support me)
- [AllDebrid](https://alldebrid.com/) (Consider using [this affiliate link](https://alldebrid.com/?uid=3n8qa&lang=en) to support me)
- [Premiumize](https://www.premiumize.me/) 
- [Debrid-Link](https://debrid-link.com/) (Consider using [this affiliate link](https://debrid-link.com/id/EY0JO) to support me)
- [Offcloud](https://offcloud.com/) (Consider using [this affiliate link](https://offcloud.com/?=06202a3d) to support me)
- [Put.io](https://put.io/)
...
Real Debrid and AllDebridoffer the best cost-value, with a large userbase ensuring extensive torrent caching and cheap price (~£2.30 / month)
Debrid-Link does offer a 300 day subscription unlike the other two which offer 180 days as a maximum. This makes it the cheapest option.
However, Debrid-Link has a smaller userbase and thus less cached torrents overall. It may still have the torrents you want. 
...
Most Debrid services restrict usage to one IP address at a time, with violations risking account bans.
Premiumize allows multiple IP addresses but limits cloud storage.

Most debrid services don't seed, and this means you aren't contributing to the torrent. 
If this is important to you, consider using Debrid-Link as it offers a seedbox. 

put.io also offers a way to seed but it has fixed limits and only for private trackers. 
It either seeds to a specific ratio or for a specific time period. This is dependent on your plan.
...
<TabItem value="offcloud" label="Offcloud">
    ...
    1. Go to https://offcloud.com/ (Consider using [this affiliate link](https://offcloud.com/?=06202a3d) to support me) 
...
<TabItem value="putio" label="put.io">
    ...
    1. Go to https://app.put.io/register and create an account
...
Now that you have the link, open Stremio, go to the addons page and click the `+ Add Addon`. Then paste the link into the box and click `Add`. 
Then a prompt will appear asking you to install the addon. Click `Install` and the addon will be installed. 
:::note
The `+ Add addon button` is only available on Stremio Web (AFAIK). To add addons through a link on other platforms, you need to paste the URL into the addon menu's search bar.
:::
...
<summary>Kitsu</summary>
    ...
    2.  
        <Tabs>
        <TabItem id="everything-else" value="Stremio Applications"> 
        Open the Stremio app, go to the addons page and click on the search bar. Paste the URL into the search bar and press enter.
        </TabItem>
        <TabItem id="web" value="Stremio Web / iOS">
        Go to the Addon menu and click on the + Add Addon button. Paste the URL into the menu that appears and click Install.
        </TabItem>
        </Tabs>
...
I highly recommend that you look at the next page ([Using Trakt](extras/trakt)) to have a more personalised experience with recommendations, watchlists, customised catalogues and more. 
